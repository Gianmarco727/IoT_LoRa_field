...
RTC_DATA_ATTR float gps2_lat = 45.838935;
RTC_DATA_ATTR float gps2_long = 12.339376;
int ID = 2;
...
//Measure moisture
  ...
  if (read_m<=2300){ //2300 arbitrary parameter calibrated from experiments to be 100% moisture
    soilMoisture=100;
  }
  else {
    soilMoisture=100*(1-(float)(read_m-2300)/(4095-2300)); //percentage of moisture calculated with linear interpolation  
  }
...
void sendReadings() {
  ...
  message = String(ID)+ "," + String(soilMoisture) + "," + String(temp)+ "," + String(hum)+ "," + String(SOH)+ "," + String(gps2_lat,6) + "," + String(gps2_long,6);
  delay(1000);
  LoRa.beginPacket();
  LoRa.print(message);
  LoRa.endPacket();
...
int getConfigData() {
  ...
  while (LoRa.available()) {
    ...
    if (LoRaData==String(ID)){ //if the packet is the same as the device ID then the packet is not for this node or it has already been set we stop listening
      return 1;
    }
    else { 
      ...
      if (deviceID==String(ID)){ //the packet is a configuration packet for this station
        gps2_lat=new_lat.toFloat();
        gps2_long=new_long.toFloat();
        freq=new_freq.toInt();
      } 
    }
  }
